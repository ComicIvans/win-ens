...
function Set-Policy {
    ...
    $backup[$PolicyInfo.Name] = (Get-ItemProperty -Path $regPath -Name 'EnableInstallerDetection' -ErrorAction SilentlyContinue).EnableInstallerDetection
    if (-not (Save-Backup)) {
        $PolicyInfo.Status = 'Completed'
        Save-GlobalInfo
        return
...
function Restore-Policy {
    ...
    $regPath = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'
    New-ItemProperty -Path $regPath -Name "EnableInstallerDetection" -Value $backup[$PolicyInfo.Name] -PropertyType DWord -Force | Out-Null
